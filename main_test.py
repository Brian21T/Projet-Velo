...
def adc_v():
    ...
    voltagex = voltage *(3.4/65535)-0.1
    voltagex = (voltagex/3.4)*100
    #print(voltage)
    #lcd_clear()
...
#---------------------------------------------------------------#
def bluetooth(pin):
    global command, x, rpm, kmh, dst, attention,latitude, longitude,d
    ...
    if uart.any():
        ...
        if (x[0] == 'd' and x[1] == 'a' and x[2] == 't' and x[3] == 'a') :
            #print("Data Request")
            uart.write("  Distance: {}Km".format(d))
...
def lcd_print(timer):
    ...
    if mode == 2:
        lcd_write('|' '+' '\xBB' '\xBB' '\xBB')
        lcd_clear()
        lcd_write("D:{:05.2f}    {:3}  {:5.2f}".format(d, C, direction_gps))
        print("D:{:05.2f}    {:3}  {:5.2f}".format(d, C, direction_gps))
    if mode == 3:
        lcd_write('|' '+' '\xBB' '\xBB' '\xBB')
        lcd_clear()
